...
warnings.filterwarnings(
    "ignore", message="resource_tracker: There appear to be", category=UserWarning
)
...
END_HOUR = 3
MESSAGE_TEXT = (
    f"🌐 Faceless Reel Generator Web UI is running!\n\nAccess it here: {FRONTEND_URL}"
)
...
def run_web_ui(headless: bool = False, dev_mode: bool = False):
    ...
    try:
        if headless:
            print(
                "Headless mode enabled. Running indefinitely. Press Ctrl+C to stop or kill the process."
            )
